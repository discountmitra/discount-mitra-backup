...
export default function ChefRequestScreen() {
  ...
  const closeSuccess = () => { setShowSuccessModal(false); navigation.goBack(); };

  // Calendar helpers
  const openDatePickerModal = () => setShowDatePicker(true);
  ...
  const navigateMonth = (dir: 'prev' | 'next') => { const d = new Date(selectedDate); d.setMonth(d.getMonth() + (dir === 'next' ? 1 : -1)); setSelectedDate(d); };
    ...
    <View style={styles.container}>
      <View style={styles.headerGradient}>
        <View style={styles.headerRow}>
          ...
          <Text style={styles.headerTitle}>Chef Request</Text>
        </View>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.fieldGroup}>
          ...
          <TouchableOpacity activeOpacity={0.85} onPress={() => setOpenVenue(!openVenue)} style={styles.select}>
            <Text style={styles.selectText}>{venue}</Text>
            <Ionicons name={openVenue ? "chevron-up" : "chevron-down"} size={18} color="#374151" />
          </TouchableOpacity>
          {openVenue && (
            <View style={styles.dropdown}>
              {VENUE_TYPES.map((t) => (
                <TouchableOpacity key={t} onPress={() => { setVenue(t); setOpenVenue(false); }} style={styles.option}>
                  <Text style={[styles.optionText, t === venue && { color: "#e91e63", fontWeight: "700" }]}>{t}</Text>
                </TouchableOpacity>
              ))}
            </View>
          )}
        ...
        <View style={styles.fieldGroup}>
          ...
          <TouchableOpacity activeOpacity={0.85} onPress={() => setOpenDays(!openDays)} style={styles.select}>
            <Text style={styles.selectText}>{days}</Text>
            <Ionicons name={openDays ? "chevron-up" : "chevron-down"} size={18} color="#374151" />
          </TouchableOpacity>
          {openDays && (
            <View style={styles.dropdown}>
              {DAYS.map((d) => (
                <TouchableOpacity key={d} onPress={() => { setDays(d); setOpenDays(false); }} style={styles.option}>
                  <Text style={[styles.optionText, d === days && { color: "#e91e63", fontWeight: "700" }]}>{d}</Text>
                </TouchableOpacity>
              ))}
            </View>
          )}
        ...
        <View style={styles.fieldGroup}>
          ...
          <TouchableOpacity activeOpacity={0.85} onPress={openDatePickerModal} style={styles.select}>
